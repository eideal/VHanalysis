...
for group in list_of_groups:
    ...
    for entry in plots:
		...
		chain.Draw('%s*%f>>%s' % (entry.variable, entry.factor, th1f_name), 'evtsel_weight*evtsel_is_WHlephad*0.02*evtsel_bjet_weight*((evtsel_is_mu || evtsel_is_el) && evtsel_passes_MV1)')
