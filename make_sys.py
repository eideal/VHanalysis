...
for lc in LimitChannels:
    ...
    for mass in br_tautau:
        ...
        if lc.category == 'zh':
            ...
            ZH.add_overall('ATLAS_UE_VH', 0.979, 1.021)
            ZH.add_overall('pdf_Higgs_ZH', mass[4][1], mass[4][0])
            #!!!!!!!!!!!!!!!! NEED TO ADD  ZH.add_shape('ATLAS_EWK', True)
...
for lc in LimitChannels:
    ...
    if lc.category == 'zh':
        MC_ZHhh = Sample('MC_ZHhh')

        for shape in mc_shape_systematics:
            MC_ZHhh.add_shape(shape[0], shape[1])

        ## Lumi uncertainty
        MC_ZHhh.add_overall('ATLAS_LUMI_2012', 0.972, 1.028)

        lc.add(MC_ZHhh)

    if lc.category == 'wh':
        MC_WHhh = Sample('MC_WHhh')

        for shape in mc_shape_systematics:
            MC_WHhh.add_shape(shape[0], shape[1])

        ## Lumi uncertainty
        MC_WHhh.add_overall('ATLAS_LUMI_2012', 0.972, 1.028)
